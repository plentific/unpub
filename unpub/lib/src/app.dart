...
class App {
  ...
  Future<shelf.Response> getVersions(shelf.Request req, String name) async {
    ...
    var versionMaps = package.versions.map((item) => _versionToJson(item, req)).toList();
    ...
    try {
      ...
      if (package != null) {
        ...
        if (package.uploaders
                ...
                .contains(uploaderEmailDomain) ==
            false) {
          throw '$uploader is not an uploader of $name';
        }

        // Check duplicated version
        var duplicated = package.versions.firstWhereOrNull((item) => version == item.version);
        if (duplicated != null) {
  ...
  Future<shelf.Response> getPackageDetail(shelf.Request req, String name, String version) async {
    ...
    var versions = package.versions.map((v) => DetailViewVersion(v.version, v.createdAt)).toList();
    versions.sort((a, b) {
